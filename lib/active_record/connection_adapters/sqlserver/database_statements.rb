      ...
      module DatabaseStatements
        ...
        def sp_executesql_sql(sql, types, params, name)
          if name == "EXPLAIN"
			...
			#Check for milli seconds and offset
			if !!sql[/(\d{4}\-\d{2}\-\d{2}\s+\d{2}\:\d{2}\:\d{2}\.\d+\+?\d+)/,1]
				while true
					sql_in = sql.dup
					sql_in.gsub!(/\d{4}\-\d{2}\-\d{2}\s+\d{2}\:\d{2}\:\d{2}\.\d+\+?\d+/, sql[/(\d{4}\-\d{2}\-\d{2}\s+\d{2}\:\d{2}\:\d{2})\.\d+\+?\d+/,1])
					unless !!sql_in[/(\d{4}\-\d{2}\-\d{2}\s+\d{2}\:\d{2}\:\d{2}\.\d+\+?\d+)/,1]
						break
					end
				end
				sql = "EXEC sp_executesql #{quote(sql_in)}"
			else
